...
class PDF extends FPDF
{
    ...
    function Total($invoice)
    {
        ...
        if ($invoice['Status'] == 'Pending' || $invoice['Status'] == 'Accepted'){
            ...
            if($this->RentalFee > $invoice['Invoice_Total'])//price reduction -- will need to change
            {
                $this->Cell(120,7,'',0,0,'C');
                $this->Cell(45,7,'Price Reduction',1,0,'L');
                $this->Cell(25,7,'($'.number_format($this->RentalFee - $invoice['Invoice_Total'] .')',2),1,1,'R');
            }
            else if ($this->RentalFee < $invoice['Invoice_Total'])//added cost -- will need to change
            {
                $this->Cell(120,7,'',0,0,'C');
                $this->Cell(45,7,'Additional Costs',1,0,'L');
                $this->Cell(25,7,'$'.number_format($invoice['Invoice_Total'] - $this->RentalFee,2),1,1,'R');
            }
        }
        //restocking cost
